/**implicit discretisation of reaction--diffusion equations
...
struct Diffusion {
	  ...
	  scalar f, f3;
	    double dt;
	      // optional
	      face vector D;  // default 1. We want D to be dependent on whether we are in f= 1 or f[] = 1
...
trace
mgstats diffusion_vof (struct Diffusion p)
{
	  ...
	  if (p.dt == 0.) {
	    ...
	    scalar f3 = p.f3, r = automatic (p.r);
		  ...
		  if (p.theta.i) {
		    ...
		    if (p.r.i)
			  ...
			  if (p.beta.i) {
			   ...
			   if (f[] == 1.) 
			     		 return poisson (f3, r, p.D, lambda);
				  else 
					 return poisson (f3, r, 0., lambda);
