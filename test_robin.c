...
#include "two-phase.h"
#include "conserving.h" //updated conserving
...
u.t[bottom] = robin(1.,0.001,0.); //robin BC - 0.015 should be the height of the flow. As B --> 0.015, the condition tends towards no slip
...
event makeslope (i++) {
		refine(y <= TRIANGLE && level < MAXLEVEL); // Refinement 
...
double pe1a = 0.0, pe2a = 0.0, pe3a = 0.0, pe4a = 0.0;
...
event energy_output_full (t+=0.01) {
	...
	//double pe1a = 0.00000000;
       ...
       //double pe1a = 0.00;
		...
		foreach(reduction(+:pe1a)) {
			double r = rho(f[]);
			pe1a +=(r*dv()*9.81*(-sin(0.268)*x+cos(0.268)*y));
		}
	}
	...
	double pe2 = 0.0;
        ...
        if (t == 0.0) {
		foreach(reduction(+:pe2a)) {
			if (x <= 1.338) {
				double r = rho(f[]);
				pe2a +=(r*dv()*9.81*(-sin(0.268)*x+cos(0.268)*y));
			}
		}
	}
	...
	static FILE * fpEE = fopen("energy_water.txt", "w");
	double ke3 = 0.0;
	double pe3 = 0.0;


        if (t == 0.0) {
		foreach(reduction(+:pe3a)) {
			if (x > 5 ){ 
				double r = rho(f[]);
				pe3a +=(r*dv()*9.81*(-sin(0.268)*x+cos(0.268)*y));
			}
		}
	}
	...
	static FILE * fpEb = fopen("energy_triangle.txt", "w");
	double ke4 = 0.0;
	double pe4 = 0.0;


        if (t == 0.0) {
		foreach(reduction(+:pe4a)) {
			if (y < TRIANGLE ){ 
				double r = rho(f[]);
				pe4a +=(r*dv()*9.81*(-sin(0.268)*x+cos(0.268)*y));
			}
		}
	}

	foreach(reduction(+:ke4) reduction(+:pe4)) {
		if (y < TRIANGLE) {
 			double r = rho(f[]);
 			ke4 += r*sq(norm(u))/2.*dv();
 			pe4 += (r*dv()*9.81*(-sin(0.268)*x+cos(0.268)*y));
		}
		//pe1 += r*dv()*9.81*cos(0.268)*y+r*dv()*9.81*sin(0.268)*x;
	}
	fprintf (fpEb, "%g %g %g\n", t, ke4, pe4-pe4a);
...
event output_files  (t += 0.004; t < 8.) { 			
			...
			output_ppm(rho, file = two, min = 1, max = 1400, n = 2048, linear = true, box = {{0,0},{12,3}}, mask = tri);
